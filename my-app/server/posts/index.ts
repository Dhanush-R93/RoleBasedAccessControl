...
.post(
    ...
    try {
      const [newPost] = await db
        ...
        .values({
          ...
          imageData: body.imageData, // Use the string sent from frontend
  ...
  },
  {
    ...
    body: t.Object({
      ...
      imageData: t.String(), // Add this to your validation
      mimeType: t.String(),
    }),
  }
)
  ...
  .get("/me", async ({ user }) => {
    if (!user) return { authenticated: false, id: null, role: null };
    try {
      const res = await db.select({id:users.id,name:users.name,email:users.email,role:users.role}).from(users).where(eq(users.id, user.id));
      return {
        authenticated: true,
        user:res[0]
      };
